...
@interface MYRandomChatPublicViewController ()<UIScrollViewDelegate,UIGestureRecognizerDelegate,THEditPhotoViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIActionSheetDelegate,UITextViewDelegate,LQPhotoPickerViewDelegate>

@property(strong,nonatomic) UIScrollView * scrollview;

@property(strong,nonatomic) UIImageView * headerimage;
@property(strong,nonatomic) MYTextView * titleview ;
...
- (void)cancel
{
    ...
    [self.contenview resignFirstResponder];
        ...
        if (self.headerimagestr ==nil &&(self.contentimagestr == nil||[self.contentimagestr isEqualToString:@""])&& (self.titleview.text == nil || [self.titleview.text isEqualToString:@""]) && (self.contenview.text ==nil ||[self.contenview.text isEqualToString:@""]))
        {
            ...
            [UIAlertViewTool showAlertView:self title:nil message:@"保存信息？" cancelTitle:@"不保存" otherTitle:@"保存" cancelBlock:^{
                ...
                if ((self.titleview.text==nil||self.contenview.text==nil)&& self.contenimagedataArr.count!=0) {
                    
                }else{
                    [MYUserDefaults setObject:self.titleview.text forKey:@"title"];
...
-(void)addtextview
{
    MYTextView *titleview = [[MYTextView alloc]initWithFrame:CGRectMake(kMargin, self.headerimage.bottom+25, MYScreenW-MYMargin, 30)];
    self.titleview = titleview;
    titleview.delegate = self;
    titleview.tag = 1;
    titleview.font = ThemeFont;
//    titleview.placeHoledrColor = UIColorFromRGB(0x4c4c4c);
    [self.scrollview addSubview:titleview];
    NSString *titlestr = [MYUserDefaults objectForKey:@"title"];
    if (titlestr !=nil &&![titlestr isEqualToString:@""]) {
        
        titleview.text = titlestr;
    }else{
        titleview.placeHoledr = @"标题";
    }
    
    
    
    UIView *line = [[UIView alloc]initWithFrame:CGRectMake(kMargin, self.titleview.bottom, MYScreenW-MYMargin, 0.5)];
    line.backgroundColor = [UIColor lightGrayColor];
    line.alpha = 0.4;
    [self.scrollview addSubview:line];
    
    
    MYTextView *contenview = [[MYTextView alloc]initWithFrame:CGRectMake(kMargin, self.titleview.bottom+kMargin, MYScreenW-MYMargin, 100)];
...
- (void)pushNextVC1
{
    ...
    if (NULLString(self.titleview.text) ||NULLString(self.contenview.text)) {
        [MBProgressHUD showHUDWithTitle:@"标题和内容都不能为空" andImage:nil andTime:1.0];
        return;
    }else if ([NSString stringContainsEmoji:self.titleview.text]||[NSString stringContainsEmoji:self.contenview.text]) {
...
//        NSString *contenstr = [NSString utf8ToUnicode:self.contenview.text];
            ...
            param[@"title"] = self.titleview.text;                      //发布标题
            param[@"classify"] = @"0";
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    [self.contenview endEditing:YES];
}
