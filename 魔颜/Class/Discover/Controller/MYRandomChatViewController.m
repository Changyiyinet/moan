//
//  MYRandomChatViewController.m
//  魔颜
//
//  Created by abc on 16/5/9.
...
@interface MYRandomChatViewController ()<searchdelegate,UITableViewDelegate,UITableViewDataSource>

@property(strong,nonatomic ) NSMutableArray *randomchatData;
...
@implementation MYRandomChatViewController

-(NSMutableArray*)randomchatData
{
    if (!_randomchatData) {
        NSMutableArray *listArr = [NSMutableArray array];
        _randomchatData = listArr;
    }
    return _randomchatData;
}
- (void)viewDidLoad {
    [super viewDidLoad];
...
-(void)requestData
{
    ...
    [MYHttpTool getWithUrl:[NSString stringWithFormat:@"%@publish/queryPublishList",kOuternet1] params:prama success:^(id responseObject) {
        ...
        if ([responseObject[@"status"] isEqualToString:@"success" ]) {
            ...
            self.randomchatData = nil;
//            [self.fmdb deleteData:@"discovergonglue"];
            
            NSArray *arr =  [MYRandonChatModle objectArrayWithKeyValuesArray:responseObject[@"publishList"]];
            self.randomchatData = (NSMutableArray*)arr;
...
//            self.tableview.footer.hidden = YES;
            ...
            [self.randomchatData addObjectsFromArray:arr];
...
{
//    return self.randomchatData.count;
    return  self.randomchatData.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.randomchatmodle = self.randomchatData[indexPath.row];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    detailVC.deleteBlock = ^(){
        ...
        MYRandonChatModle *model = self.randomchatData[indexPath.row];
        [self.randomchatData removeObject:model];
        [self.tableview reloadData];
