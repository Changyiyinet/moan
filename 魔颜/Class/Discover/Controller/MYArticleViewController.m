...
@interface MYArticleViewController ()<UITableViewDataSource,UITableViewDelegate,UITextViewDelegate>
...
- (void)setUI
{
    ...
    tableview.frame = CGRectMake(0, 0, MYScreenW, self.view.height - 50);
    ...
    UIImage *image = [UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",kOuternet1,self.headImage]]]];
    ...
    if ([self.TYpe isEqualToString:@"pul"]) {
        imgView.image  = [UIImage imageWithData:self.headimagedata];
    }else{
        imgView.image = image;
    }
    ...
    self.tableView.contentInset = UIEdgeInsetsMake(imgView.height, 0, 0, 0);
...
- (void)setupTableViewHeadUI
{
    ...
    //点赞小图片
    UIScrollView *praseView = [[UIScrollView alloc]init];
    [view addSubview:praseView];
    praseView.delegate = self;
    praseView.scrollEnabled = YES;
    praseView.showsHorizontalScrollIndicator = NO;
...
//                // 详情页面的点赞的小头像
//                NSArray *arr = responseObject[@"commentUser"];
//                for (NSDictionary *dict in arr) {
//                    [self.picArr addObject:[dict objectForKey:@"pic"]];
//                }
               [self setHeadData];
...
//        self.praseView.frame = CGRectMake(0, bgSview.bottom + 30, MYScreenW, 50);
//        
...
//            userPic.width = 30;
//            userPic.height = 30;
//            [userPic setCircleHeaderWithURL:[NSString stringWithFormat:@"%@%@",kOuternet1,self.picArr[i]]];
//            [self.praseView addSubview:userPic];
//        }
//      }
...
- (void)sendMessage:(MYCircleBtn *)btn
{
    if (!MYAppDelegate.isLogin) {
    ...
    }else if(NULLString(self.textView.text)){
        ...
        if (self.sendBtn.tag != -1) {
            ...
            MYCommentsModel *commentModel = self.commentArr[self.sendBtn.tag][btn.ID];
...
//    [self.view endEditing:YES];
    ...
    if (contentOffsetY > -ScrollHeadViewHeight && contentOffsetY<0) {
